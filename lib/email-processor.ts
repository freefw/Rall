      ...
      const processMailbox = (index: number) => {
        ...
        imap.openBox(mailbox, false, (openErr) => {
          if (openErr) {
            this.onLog(
              "WARNING",
              `无法打开邮箱文件夹 ${mailbox}: ${openErr.message}`,
              this.account.email,
            )
          ...
          this.onLog("INFO", `打开邮箱文件夹 ${mailbox}`, this.account.email)

          imap.search(["UNSEEN"], (searchErr, results) => {
            if (searchErr) {
              this.onLog(
                "ERROR",
                `搜索 ${mailbox} 未读邮件失败: ${searchErr.message}`,
                this.account.email,
              )
            ...
            if (!results || results.length === 0) {
              this.onLog("INFO", `${mailbox} 没有未读邮件`, this.account.email)
              closeMailbox(() => processMailbox(index + 1))
              return
            }
            ...
            this.onLog(
              "INFO",
              `在 ${mailbox} 中发现 ${results.length} 封未读邮件`,
              this.account.email,
            )
            ...
            fetch.once("error", (fetchErr) => {
              this.onLog(
                "ERROR",
                `获取 ${mailbox} 邮件失败: ${fetchErr.message}`,
                this.account.email,
              )
            })
            ...
            fetch.once("end", () => {
              Promise.allSettled(messagePromises).finally(() => {
                ...
                this.onLog(
                  "INFO",
                  `${mailbox} 处理完毕，处理 ${processedInMailbox} 封邮件`,
                  this.account.email,
                )
                closeMailbox(() => processMailbox(index + 1))
              })
